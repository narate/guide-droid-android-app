...
import com.jwetherell.augmented_reality.ui.BigRadar;
...
public class RadarView extends View {
	...
	private static final String startKM = "0";
	private static final String endKM = FORMAT
			.format(AugmentedReality.MAX_ZOOM);
	private static final int leftBound = 12;
	private static final int rightBound = 79;
	private static final int conflictHeight = 82;
	private static BigRadar radar;	
	...
	public RadarView(Context context, boolean useCollisionDetection) {
		super(context);
		radar = new BigRadar((SplashScreen.getWidth()/2)-10);
		RadarView.useCollisionDetection = useCollisionDetection;
	}
		...
		if (drawing.compareAndSet(false, true)) {
			if (startTxtContainter == null) {
				...
				startTxtContainter = new PaintablePosition(startTextBlock,
						...
						(canvas.getHeight() / 100 * labelY), 0, 1);
			}
			...
			if (useCollisionDetection){
				collection = adjustForCollisions(canvas, collection);
			}
	...
	private static List<Marker> adjustForCollisions(Canvas canvas,

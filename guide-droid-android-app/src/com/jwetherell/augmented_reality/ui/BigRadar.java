...
import com.narate.ar.fork_framework.SplashScreen;
 ...
 */
public class BigRadar {
	public static float RADIUS = 0;

	private static final int LINE_COLOR = Color.argb(150, 0, 220, 0);
	private static final float PAD_X = 10;
	private static final float PAD_Y = 20;
	private static final int RADAR_COLOR = Color.argb(200, 0, 0, 0);
	...
	public BigRadar(int radius) {
		RADIUS = radius;
		cx = (int) ((SplashScreen.getWidth() / 2) - 60);
		cy = cx + 10;
	...
	private void drawRadarPoints(Canvas canvas) {
		...
		if (pointsContainer == null) {
			pointsContainer = new PaintablePosition(radarPoints, cx, cy,
					-ARData.getAzimuth(), 4);
		} else {
			pointsContainer.set(radarPoints, cx, cy, -ARData.getAzimuth(), 4);
		}
	...
	private static String formatDist(float meters) {
		if (meters < 1000) {
			...
			return formatDec(meters / 1000f, 1) + " กม.";
		} else {
			return ((int) (meters / 1000f)) + " กม.";
		}
	}

...
public class Marker implements Comparable<Marker> {
	...
	// Symbol's (circle in this case) X, Y, Z position relative to the camera's
	// view
	...
	private String id = "", category = "", title = "", latitude = "",
			longitude = "", description = "", owner = "";
	private Place place;
	...
	public synchronized void setPlace(String location_id,
			String location_cate_id, String title, String latitude,
			String longitude, float distance, String description,
			String owner_id) {
		this.place = new Place(location_id, location_cate_id, title, latitude,
				longitude, distance, description, owner_id);
	}
	...
	public synchronized Vector getScreenPosition() {
		...
		// If the marker has been visible, use the text box to offset the
		// position.
		if (textBox != null)
	...
	private synchronized void updateView() {
		...
		if (x1 >= 0 && x2 <= cam.getWidth() && y1 >= 0 && y2 <= cam.getHeight()) {
			isInView = true;
		}
	}
	...
	public synchronized boolean isPointOnMarker(float x, float y) {
		...
		if (x >= (myX - adjWidth) && x <= (myX + adjWidth)
				&& y >= (myY - adjHeight) && y <= (myY + adjHeight))
			return true;
	...
	protected synchronized void drawTouchZone(Canvas canvas) {
		...
		if (touchBox == null)
			touchBox = new PaintableBox(width, height);
		else
			touchBox.set(width, height);

		if (touchPosition == null)
			touchPosition = new PaintablePosition(touchBox, adjX, adjY,
					currentAngle, 1);
		else
			touchPosition.set(touchBox, adjX, adjY, currentAngle, 1);
	...
	protected synchronized void drawIcon(Canvas canvas) {
		...
		if (symbolContainer == null)
			...
			symbolContainer
					.set(gpsSymbol, symbolArray[0], symbolArray[1], 0, 1);
	...
	private synchronized void drawText(Canvas canvas) {
		...
		if (textBox == null)
			textBox = new PaintableBoxedText(textStr,
					Math.round(maxHeight / 2f) + 1, 500); // Math.round(maxHeight / 2f) + 1
		else
			textBox.set(textStr, Math.round(maxHeight / 2f) + 1, 500); // 300
		...
		if (textContainer == null)
			textContainer = new PaintablePosition(textBox, x, y, angle, 1);
		else
			textContainer.set(textBox, x, y, angle, 1);
